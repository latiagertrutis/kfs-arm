...
@@latex:\leavevmode\\@@
In a embedded project (specially a bare-metal one) it is very easy to get messy. One project can support many board with different HALs, startup codes and so on. So here is a project structure I suggest to keep things compartmentalized:
